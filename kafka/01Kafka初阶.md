...
* **日志处理**
	* **可以使用消息队列作为临时存储，或者一种通信管道**。比如大型电商网站（淘宝、京东、国美、苏宁...）、App（抖音、美团、滴滴等）等需要分析用户行为
...
* **消息队列的模式**
	* **点对点：一个消费者消费一个消息，消息被消费以后，消息队列中不再有存储**  
		![](assets/01Kafka初阶/file-20250825142045595.png)
		![](assets/01Kafka初阶/file-20250825142227520.png)
	* **发布订阅：多个消费者可以消费一个消息**
		![](assets/01Kafka初阶/file-20250825142118228.png)
		![](assets/01Kafka初阶/file-20250825142309105.png)
### 4.kafka简介

###### 什么是kafaka
* Apache Kafka是一个**分布式流平台**。一个分布式的流平台应该包含3点关键的能力：
	1. **发布和订阅流数据流，类似于消息队列或者是企业消息传递系统**
	2. **以容错的持久化方式存储数据流**
	1. **处理数据流**

###### kafka的应用场景
![](assets/01Kafka初阶/file-20250825143750562.png)

###### kafka的优势
![](assets/01Kafka初阶/file-20250825144030292.png)
![](assets/01Kafka初阶/file-20250825144226762.png)
## 二、Kafka集群搭建
...
### 1.Kafka的生产者/消费者/工具
![](assets/01Kafka初阶/file-20250826134002004.png)
* 安装Kafka集群，可以测试以下
...
1. 创建连接
	...
	* **key.serializer：Kafka中的消息是以key、value键值对存储的，而且生产者生产的消息是需要在网络上传传输的，所以此处就是指定key的序列化方式**，这里指定的是StringSerializer方式，就是以字符串方式发送（将来还可以使用其他的一些序列化框架：Google ProtoBuf、Avro）
	* value.serializer：同上  
2. 创建一个**生产者对象KafkaProducer（kafka当中存在topic里的数据都是以key-value的形式存储的）**
3. 调用**send方法发送消息（其中的消息通过ProducerRecord封装，封装是key-value键值对）**
4. 调用**Future.get表示等带服务端的响应**
5. 关闭生产者
...
public class KafkaProducerTest {
    public static void main(String[] args) throws ExecutionException, InterruptedException {
        ...
        for(int i = 0; i < 100; ++i) {
            // 构建一条消息，直接new ProducerRecord,其中第一个参数为topic名字
