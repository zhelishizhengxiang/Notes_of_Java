# 一、服务管理类命令
![](assets/第3章%20系统命令/file-20251112235343701.png)
![](assets/第3章%20系统命令/file-20251112235913027.png)


后面进行java开发之类的，需要将下列服务进行关闭   
![](assets/第3章%20系统命令/file-20251113000140474.png)
* 将来虚拟机作为一个ip时，需要关闭以下服务
![](assets/第3章%20系统命令/file-20251122165049051.png)
![](assets/第3章%20系统命令/file-20251122165533096.png)
# 二、系统当前时间 
date命令：
![](assets/第3章%20系统命令/file-20251123224846268.png)
![](assets/第3章%20系统命令/file-20251123225447612.png)
* 标准格式：date +%F：显示就是经常使用的年月日格式——2023-11-09

![](assets/第3章%20系统命令/file-20251123225654840.png)
![](assets/第3章%20系统命令/file-20251123225759154.png)
...
```
这将会输出 `My name is John` 和一个换行符。在输出字符串时，**输出变量使用 `$` 符号加上变量名即可。**
...
* 如果想要输出\t而不是让他成为制表符时，前面要加转义字符\\\\，表示想要输出\t本身
...
# 八、grep命令（过滤查找）
![](assets/第3章%20系统命令/file-20251128213157642.png)
* 常与管道符联合使用，用于查找包含指定内容的东西
...
![](assets/第3章%20系统命令/file-20251210232037137.png)
# sleep命令
...
# 十三、top命令
**`top` 命令是用于查看正在运行的系统进程信息的命令**。它会实时**动态地显示系统资源的使用情况，如 CPU 占用率、内存使用情况、进程情况等。通常用于系统监控和性能调优**。
...
![](assets/第3章%20系统命令/file-20251210230908176.png)
* 对于程序员来说，应该重点关注以上红框中的内容，已用空间超过80%则需要重点注意，以防磁盘已满导致数据无法写入而丢失。
*  -h参数表示以人们较容易阅读的格式自行显示
